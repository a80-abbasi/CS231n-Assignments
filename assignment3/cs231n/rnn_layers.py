...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        c_t = caches[t]
        # add two branch of gradient
        dh_t = dh[:, t, :] + dh_t
        dx_t, dh_t_1, dWx_t, dWh_t, db_t = rnn_step_backward(dh_t, c_t)
        dx[:, t, :] = dx_t
        db += db_t
        dWx += dWx_t
        dWh += dWh_t
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    assert (x < W.shape[0]).all(), "Each element idx of x muxt be in the range 0 <= idx < V"
    out = W[x]
    cache = W, x
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = prev_c, prev_h, x, Wx, Wh, I, F, O, G, next_c_tanh
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    prev_c, prev_h, x, Wx, Wh, I, F, O, G, next_c_tanh = cache
    dO = dnext_h * next_c_tanh
    dnext_c_tanh = dnext_h * O
    # d/dz tanh(z) = 1 - tanh(z) ** 2
    dnext_c = dnext_c + dnext_c_tanh * (1 - next_c_tanh ** 2)
    dF = dnext_c * prev_c
    dprev_c = dnext_c * F
    dI = dnext_c * G
    dG = dnext_c * I
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dh_next = dh_next + dh[:, t, :]
        dx_t, dprev_h_t, dprev_c_t, dWx_t, dWh_t, db_t = lstm_step_backward(dh_next, dc_next, caches[t])
        dx[:, t, :] = dx_t
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
